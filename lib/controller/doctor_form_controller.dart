...
class DoctorFormController {
  ...
  static const String URL =
      "https://script.google.com/macros/s/AKfycbwYDXqrjCHJo2K6wCFdevVMHrWraPDdRyo0U7vsDuaYVyc6bT8S/exec";
  ...
  void submitForm(
      DoctorFormData feedForm, void Function(String) callback) async {
    try {
  ...
  Future<List<DoctorFormData>> getFeedList() async {
    return await http.get(URL).then((response) {
      var jsonFeedResult = convert.jsonDecode(response.body) as List;
      print(
          "response: ${jsonFeedResult.map((v) => DoctorFormData.fromJson(v).toJson())}");

      return jsonFeedResult
          .map((json) => DoctorFormData.fromJson(json))
          .toList();
    });
  }
}
